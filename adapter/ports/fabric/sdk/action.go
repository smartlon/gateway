...
func (action *Action) Set(channelID, chaincodeID string, argsArray []Args) {
	...
	action.ChaincodeID = chaincodeID
	action.Args = argsArray
}
...
func (action *Action) Initialize() error {
	...
	if Config().SelectionProvider != AutoDetectSelectionProvider {
	...
	sdk, err := fabsdk.New(Config().ConfigProvider, opts...)
	if err != nil {
	...
	if Config().IsLoggingEnabledFo {
		log.Debug("All Peers:")
		for orgID, peers := range allPeersByOrg {
			log.Debugf("Org: %s\n", orgID)
			for i, peer := range peers {
				log.Debugf("-- Peer[%d]: MSPID: %s, URL: %s\n", i, peer.MSPID(), peer.URL())
			}
		}
	}

	// Filter peers by specified peers/orgs
	peers, err := action.getPeers(allPeers, Config().PeerURLs, Config().OrgIDs)
	if err != nil {
		return err
	}
...
func (action *Action) ChannelClient(...channel.ClientOption) (*channel.Client, error) {
	...
	channelProvider := func() (context.Channel, error) {
		return contextImpl.NewChannel(session, action.ChannelID)
	}
...
func (action *Action) OrgAdminChannelClient(orgID string) (*channel.Client, error) {
	...
	log.Debugf("Creating new channel client for channel [%s] and org [%s] ...", channelID, orgID)
...
func (action *Action) ClientForUser(channelID string, user mspapi.SigningIdentity) (*channel.Client, error) {
	log.Debugf("create resmgmt client for user [%s] in org [%s]...", user.Identifier().ID, user.Identifier().MSPID)
	session, err := action.context(user)
	if err != nil {
		return nil, errors.Errorf("error getting session for user [%s,%s]: %v", user.Identifier().MSPID, user.Identifier().ID, err)
	}
...
func (action *Action) ChannelMgmtClientForUser(channelID string, user mspapi.SigningIdentity) (*channel.Client, error) {
	log.Debugf("create channel client for user [%s] in org [%s]...", user.Identifier().ID, user.Identifier().MSPID)
	session, err := action.context(user)
	if err != nil {
		return nil, errors.Errorf("error getting session for user [%s,%s]: %v", user.Identifier().MSPID, user.Identifier().ID, err)
	}
...
func (action *Action) context(user mspapi.SigningIdentity) (context.ClientProvider, error) {
	...
	if session == nil {
		...
		log.Debugf("Created session for user [%s] in org [%s]", user.Identifier().ID, user.Identifier().MSPID)
...
func (action *Action) OrgUser(orgID, username string) (mspapi.SigningIdentity, error) {
	...
	log.Infof("action.OrgUser: %s",orgID)
	mspClient, err := msp.New(action.sdk.Context(), msp.WithOrg(orgID))
	if err != nil {
		return nil, errors.Errorf("error creating MSP client: %s", err)
	}
...
func (action *Action) PeerConfig() (*fab.PeerConfig, error) {
	...
	if !ok {
		return nil, errors.Errorf("Error reading peers config for %s", action.OrgID())
	}
