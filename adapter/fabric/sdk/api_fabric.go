...
func RegisterUser(username, pwd string) (
	...
	if username == "" && pwd == "" {
		err = fmt.Errorf("must specify the username and pwd")
		return
	}
	action, err := newUserAction()
	if err != nil {
		log.Errorf("Error while initializing userAction: %v", err)
		return
	}

	defer action.Terminate()

	_, err = action.registerUser(username,pwd)
	if err != nil {
		log.Errorf("Error while calling action.registerUser(): %v", err)
